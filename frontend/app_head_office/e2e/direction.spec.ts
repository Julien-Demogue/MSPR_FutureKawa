...
test.describe('E2E-03 — Dashboard Direction', () => {
  ...
  test('E2E-03-A : filtre pays → filtre entrepôt se met à jour', async ({ page }) => {
    ...
    expect(brazilOptions.some(o => o.includes('Fazenda'))).toBeTruthy();
    ...
    expect(ecuadorOptions.some(o => o.includes('Fazenda'))).toBeFalsy();
  });
  ...
  test('E2E-03-D : tableau Stocks est trié FIFO par défaut', async ({ page }) => {
    ...
    const dates = await page.locator('tbody tr td:nth-child(5)').allTextContents();
  ...
  test('E2E-03-F : changement de période sur les graphiques', async ({ page }) => {
    ...
    // Vérifier que les labels de l'axe X sont en format horaire (HH:00)
    // Les labels Recharts sont dans des éléments SVG <text>
    await expect(page.locator('svg text').filter({ hasText: /^\d{2}:00$/ }).first())
